...
class CocoPETCT:
    def __init__(self, img_folder, ann_folder, ann_dict, training, transforms=None, return_masks=True):
        self.coco = ann_dict
    ...
    def __getitem__(self, idx):
        ...
        if self.training:
            ...
            norm = plt.Normalize(vmin = 0, vmax = 6) # normally radiologists view images at this suv norm
        ...
        img = Image.fromarray(np.uint8(img*255), 'RGB') #input has to be PIL for rest of detr dataloading pipeline!...
        w, h = img.size
...
#             labels = torch.tensor([ann['category_id'] for ann in ann_info['segments_info']], dtype=torch.int64)
        ...
        #petct coco dataset presaved bbox in xywh
        target["boxes"] = torch.tensor([box_xywh_to_xyxy(ann['bbox']) for ann in ann_info['segments_info']], dtype=torch.int64) 
...
def build(image_set, args):
    img_folder_root = Path(args.coco_petct_path)
    ...
    ann_folder = img_folder_path # binary masks pre-saved in same .npy as the image
    ann_file = ann_folder_root / ann_file
    
    if image_set == "train":
        # customized train val split
        print('get train val split')
        train_dict, val_dict = train_val_split(ann_file, args.train_split, args.cross_val, args.val_fold)
        
        print('building petct train dataset for coco')
        train_dataset = CocoPETCT(img_folder_path, ann_folder, train_dict, training = True,
                               transforms=make_coco_transforms("train"), return_masks=args.masks)
        
        print('building petct val dataset for coco')
        val_dataset = CocoPETCT(img_folder_path, ann_folder, val_dict, training = False,
